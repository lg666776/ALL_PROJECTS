...
BUY_ORDER_DETAILS_FILE = 'tatam_buy_order_details.pkl'
SELL_ORDER_DETAILS_FILE = 'tatam_sell_order_details.pkl'
EXECUTED_HIGH_VALUES_FILE = 'tatam_executed_high_values.pkl'
EXECUTED_LOW_VALUES_FILE = 'tatam_executed_low_values.pkl'

# Get the current date and time
current_datetime = datetime.datetime.now()

# Define the time range for data removal (15:30 to 23:50)
start_time_1 = current_datetime.replace(hour=15, minute=30, second=0, microsecond=0)
end_time_1 = current_datetime.replace(hour=23, minute=50, second=0, microsecond=0)

# Check if the current time is within the specified time range
if start_time_1 <= current_datetime <= end_time_1:
    # Delete the data files
    if os.path.exists(BUY_ORDER_DETAILS_FILE):
        os.remove(BUY_ORDER_DETAILS_FILE)
        print("Buy order details file removed.")
    if os.path.exists(SELL_ORDER_DETAILS_FILE):
        os.remove(SELL_ORDER_DETAILS_FILE)
        print("Sell order details file removed.")
    if os.path.exists(EXECUTED_HIGH_VALUES_FILE):
        os.remove(EXECUTED_HIGH_VALUES_FILE)
        print("Executed high values file removed.")
    if os.path.exists(EXECUTED_LOW_VALUES_FILE):
        os.remove(EXECUTED_LOW_VALUES_FILE)
...
def get_order_book_loop():
    ...
    while True:
        ...
        time.sleep(20)  # Sleep for 5 seconds at the end of the loop
        ...
        try:
            ...
            with open('data.pickle', 'rb') as file:
                try:
                    ...
                    print("Ran out of input while reading data.pickle.")
...
def buy_condition_check():
    global tata_motors_lp_value, store_high, buy_order_details, order_status_counts, total_count
...
def exit_trade_1_loop():
    global buy_order_details, tata_motors_lp_value, store_low
...
def exit_trade_2_loop():
    global buy_order_details, tata_motors_lp_value, lowest_point
...
def sell_condition_check():
    global tata_motors_lp_value, store_low_sell, sell_order_details, order_status_counts, total_count
    ...
    while not sell_order_executed and retries < max_retries:
        if tata_motors_lp_value is not None and store_low_sell is not None:
            if isinstance(tata_motors_lp_value, (int, float)) and isinstance(store_low_sell, (int, float)):
...
def exit_trade_1_loop_sell():
    global sell_order_details, tata_motors_lp_value, store_high_sell
...
def exit_trade_2_loop_sell():
    global sell_order_details, tata_motors_lp_value, highest_point
...
def fetch_candle_data_loop():
    ...
    # Define the time interval for the 5-minute candles
    start_time = datetime.datetime.strptime('08-08-2023 09:15:00', '%d-%m-%Y %H:%M:%S')
    end_time = datetime.datetime.strptime('09-08-2023 23:25:00', '%d-%m-%Y %H:%M:%S')
...
def get_next_interval_mark(interval):
    ...
    next_interval_mark += datetime.timedelta(minutes=5, seconds=10)
