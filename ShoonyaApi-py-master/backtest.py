...
EXECUTED_HIGH_VALUES_FILE = 'tm_executed_high_values.pickle'
EXECUTED_LOW_VALUES_FILE = 'tm_executed_low_values.pickle'
...
def exit_trade_1_loop():
    global buy_order_details, tata_motors_lp_value, store_low, exit1_buy_order_details, exit1_orderno
...
def exit_trade_2_loop():
    global buy_order_details, tata_motors_lp_value, lowest_point, exit2_buy_order_details, exit2_orderno
    ...
    while not sell_order_executed and retries < max_retries:
        ...
        if tata_motors_lp_value is not None and store_low_sell is not None:
            if isinstance(tata_motors_lp_value, (int, float)) and isinstance(store_low_sell, (int, float)):
                ...
                if tata_motors_lp_value < entry_price and tata_motors_lp_value > (entry_price - 5):
                    if store_low_sell not in executed_low_values:
                        if not buy_order_details:  # Check if buy_order_details is empty
                            if not sell_order_details:  # Check if sell_order_details is empty
                                # Check if the current time is before 14:20
                                current_time = datetime.datetime.now().time()
                                if current_time < datetime.time(18, 20):
                                    if total_count <= 5:
...
def exit_trade_1_loop_sell():
    global sell_order_details, tata_motors_lp_value, store_high_sell, exit1_sell_order_details, exit1_sell_orderno
...
def exit_trade_2_loop_sell():
    global sell_order_details, tata_motors_lp_value, highest_point, exit2_sell_order_details, exit2_sell_orderno
